...
export function CampaignCard({ campaign, onApplyAsLeader, onApplyAsMember, onCardClick }: CampaignCardProps) {
  return (
    <div
      className="bg-surface border border-border rounded-xl p-6 hover:border-border-strong hover:bg-surface-muted transition-all cursor-pointer"
      ...
      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-3">
            <h3 className="text-xl font-semibold text-text line-clamp-2 mb-2 sm:mb-0">
              {campaign.title}
            </h3>
          ...
          <div className="text-sm text-text-subtle mb-3 space-y-1">
            <div>
              <span className="font-medium text-text-muted">광고주:</span> {campaign.advertiser.user.name} ·{' '}
              <span className="font-medium text-text-muted">장소:</span> {campaign.place.name}
            ...
            <div>
              <span className="font-medium text-text-muted">신청 마감:</span> {new Date(campaign.applicationDeadline).toLocaleDateString('ko-KR')} ·{' '}
              <span className="font-medium text-text-muted">체험 기간:</span> {new Date(campaign.startDate).toLocaleDateString('ko-KR')} ~ {new Date(campaign.endDate).toLocaleDateString('ko-KR')}
            </div>
          </div>
        </div>
        ...
        <div className="flex-shrink-0">
          <div className="grid grid-cols-1 gap-3 text-sm">
            <div className="text-text-subtle">
              <span className="font-medium text-text-muted">팀 현황:</span> {campaign._count.teams} / {campaign.targetTeamCount}팀
            </div>
            <div className="text-text-subtle">
              <span className="font-medium text-text-muted">팀당 인원:</span> 최대 {campaign.maxMembersPerTeam}명
            </div>
          </div>
