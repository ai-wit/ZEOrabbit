...
export default function NewExperienceApplicationClient({ advertiserInfo }: Props) {
  ...
  useEffect(() => {
    ...
    if (placeType && (placeType === 'OPENING_SOON' || placeType === 'OPERATING')) {
      console.log('Setting selectedPlaceType:', placeType);
  ...
  // selectedPlaceType이 변경될 때 pricingPlans 로드
  useEffect(() => {
    console.log('useEffect triggered for selectedPlaceType:', selectedPlaceType);
    if (!selectedPlaceType) {
      console.log('selectedPlaceType is null, skipping pricing plans load');
      return;
    }

    const loadPricingPlans = async () => {
      console.log('Loading pricing plans for placeType:', selectedPlaceType);
      try {
        const response = await fetch(`/api/advertiser/experience/pricing-plans?placeType=${selectedPlaceType}`);
        if (response.ok) {
          const plans = await response.json();
          console.log('Loaded pricing plans for', selectedPlaceType, ':', plans.length, 'plans');
          setPricingPlans(plans);
        } else {
          console.error('Pricing plans API failed:', response.status);
        }
      } catch (error) {
  ...
  // pricingPlans가 로드된 후 URL에서 selectedPlan 복원
  useEffect(() => {
    const pricingPlanId = searchParams.get('pricingPlanId');
    if (pricingPlanId && pricingPlans.length > 0 && !selectedPlan) {
      const plan = pricingPlans.find((p: PricingPlan) => p.id === pricingPlanId);
      if (plan) {
        console.log('Setting selectedPlan from URL:', plan);
        setSelectedPlan(plan);
      }
    }
  }, [searchParams, pricingPlans, selectedPlan]);

  // 매장 유형 선택 시 요금제 로드
  const handlePlaceTypeSelect = async (placeType: PlaceType) => {
    setSelectedPlaceType(placeType);
    // pricingPlans는 useEffect에서 자동으로 로드됨
  };
  ...
  const handlePayment = async () => {
    ...
    // 결제 진행 전에 URL 파라미터에 현재 상태 저장
    if (typeof window !== 'undefined') {
      const url = new URL(window.location.href);
      url.searchParams.set('placeType', selectedPlaceType);
      url.searchParams.set('pricingPlanId', selectedPlan.id);
      url.searchParams.set('step', '4'); // 결제 진행 중
            ...
            <div className="flex justify-between pt-6">
              ...
              <Button
                ...
                onClick={async () => {
                  try {
                    // 추가 정보 저장 API 호출
                    const response = await fetch('/api/advertiser/experience/applications/complete', {
                      method: 'POST',
                      headers: {
                        'Content-Type': 'application/json',
                      },
